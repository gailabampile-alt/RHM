...
$pdf->ln(5);
$txt = "LISTE DES ENFANTS PRISES EN CHARGE PAR AGENTS";
...
*/
    $header = array('MATRICULE', 'NOM PARENT', 'NOM ENFANT','SEXE ENFANT','DATE NAISSANCE');
    // Data loading
    $result = $db->prepare('SELECT  t_enfants_agent.id_enf,t_enfants_agent.agent_ID,t_enfants_agent.nom_enf,t_enfants_agent.postnom_enf,
    t_enfants_agent.postnom_enf,t_enfants_agent.prenom_enf,t_enfants_agent.sexe_enf,t_enfants_agent.creerPar,
    t_enfants_agent.dateCreate,t_enfants_agent.modifierPar,t_enfants_agent.dateModif,t_enfants_agent.dateNaiss_enf,
    ...
    INNER JOIN bdd_paie.t_agent ON t_enfants_agent.agent_ID = t_agent.matricule ORDER BY Matricule');
    $result ->execute();
    $last_matric = NULL;
    while($row = $result->fetch())
    {
        $matric = $row['agent_ID'];
        $nomComplet_parent = $row['nom_ag'].' '.$row['postnom_ag'].' '.$row['prenom_ag'];
        $nomComplet_enfant = $row['nom_enf'].' '.$row['postnom_enf'].' '.$row['prenom_enf'];
        $sexe_enf = $row['sexe_enf'];
        $dateNaiss_enf = $row['dateNaiss_enf'];
        
        $data[] = array($matric, $nomComplet_parent,$nomComplet_enfant, 
                        $sexe_enf, $dateNaiss_enf );
