    ...
    try {
        ...
        if(isset($_POST['addsanction'])){
          ...
          //  $excercice = validation_donnees($_POST['excercice']);
            ...
            if($_FILES['fich_doc_sanct']['size'] < 10000000){
                ...
                if(in_array(strtolower($fichiersanct_Path),$valid)){
                    ...
                    header('location:accueil.php?page=Disciplinaire');
                    exit;
                }
            }
          ...
          //  $statut = "act";
            ...
            {
                try {
                    $reqAddVehicule = $db->prepare (" INSERT INTO bdd_paie.t_sanct_agent (`id_typesanct`, `matricule`, `ref_sanct`, `observation`, `date_debut`, `date_fin`, `sanction`, `sanction_byte`, `creerPar`, `datecreat`, `modifierPar`, `dateModif`) 
                    VALUES (:sanct,:matricule,:ref_sanct,:observation,:date_debut,:date_fin,:fichier,:fichier_byte,:creerPar,:datecreat,:modifierPar,:dateModif)");
