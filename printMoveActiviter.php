...
/*------------------------------------------------------------
 1) Empêcher l’erreur FPDF “headers already sent”
-------------------------------------------------------------*/
ob_start();
ini_set('display_errors', 0);

/*------------------------------------------------------------
 2) Conversion UTF‑8 → ISO‑8859‑1 sécurisée pour FPDF
   (identique à printMoveFonct)
-------------------------------------------------------------*/
function toLatin1($str)
{
    ...
    if ($converted === false) $converted = utf8_decode($str);
...
/*------------------------------------------------------------
 4) Date française (identique à printMoveFonct)
-------------------------------------------------------------*/
function dateFR($d)
{
...
/*------------------------------------------------------------
 5) Récupérer nom complet de l’agent (identique à printMoveFonct)
-------------------------------------------------------------*/
$stmt = $db->prepare("
...
/*------------------------------------------------------------
 6) Classe PDF avec le meme entete que printMoveCarriere.php
-------------------------------------------------------------*/
class PDF extends FPDF
{
    ...
    function InfoAgent($matricule,$nom) {
        ...
        $this->Cell(0,10,toLatin1("CHRONOLOGIE DES MOUVEMENTS D'ACTIVITÉ"),0,1,'C');
    ...
    function EnteteTableau() {
        ...
        $this->Cell(210,10,toLatin1("Chronologie d'activité"),1,1,'C');

        $this->SetX($x + 40);
        $this->Cell(110,10,toLatin1("Activité"),1,0,'L');
        $this->Cell(50,10,toLatin1("Date Début"),1,0,'C');
        $this->Cell(50,10,toLatin1("Date Fin"),1,1,'C');
    }

    function LigneTableau($num,$activ,$deb,$fin) {
        ...
        $this->Cell(110,10,toLatin1($activ),1,0,'L');
...
/*------------------------------------------------------------
 7) Génération PDF (identique au style de printMoveFonct)
-------------------------------------------------------------*/
$pdf = new PDF('L','mm','A4');
$pdf->AliasNbPages();
$pdf->showHeader = true;
...
/*------------------------------------------------------------
 8) Charger mouvements d’ACTIVITÉ (équivalent fonction mais table différente)
-------------------------------------------------------------*/
$q = $db->prepare("
    SELECT
        aga.code_activ_ID,
    ...
    INNER JOIN bdd_paie.t_activite AS act 
        ON aga.code_activ_ID = act.code_activ
    WHERE aga.agent_ID = :m
    ORDER BY aga.dateDebut ASC
");
...
while ($row = $q->fetch(PDO::FETCH_ASSOC)) {
    ...
    $activ = $row['code_activ_ID']." \n ".$row['libelle_activ'];
    $deb   = dateFR($row['dateDebut']);
    $fin   = dateFR($row['dateFin']);

    $pdf->LigneTableau($i,$activ,$deb,$fin);
    $i++;
}
...
$pdf->Cell(0,8,toLatin1("KASONGO NKOY"),0,1,'C');

ob_end_clean();
$pdf->Output('I','etat_carriere_activite.pdf');
