...
/*------------------------------------------------------------
  1) Empêcher l’erreur FPDF “Certaines données déjà envoyées”
-------------------------------------------------------------*/
ob_start();
...
/*------------------------------------------------------------
  4) Date en français : 2024-03-02 → 02 mars 2024
-------------------------------------------------------------*/
function dateFR($d)
{
...
/*------------------------------------------------------------
  5) Récupérer nom complet de l’agent
-------------------------------------------------------------*/
$stmt = $db->prepare("
...
class PDF extends FPDF
{
    ...
    function InfoAgent($matricule,$nom) {
        ...
        $this->Cell(0,10,toLatin1("CHRONOLOGIE DES MOUVEMENTS DE FONCTION"),0,1,'C');
    ...
    function EnteteTableau() {
        ...
        $this->Cell(210,10,toLatin1("Chronologie de fonction"),1,1,'C');

        $this->SetX($x + 40);
        $this->Cell(110,10,toLatin1("Fonction"),1,0,'L');
        $this->Cell(50,10,toLatin1("Date Début"),1,0,'C');
        $this->Cell(50,10,toLatin1("Date Fin"),1,1,'C');
    }

    function LigneTableau($num,$fonction,$deb,$fin) {
...
/*------------------------------------------------------------
  8) Charger les mouvements de FONCTION
-------------------------------------------------------------*/
$q = $db->prepare("
    SELECT 
        agf.fonction_ID,
...
while ($row = $q->fetch(PDO::FETCH_ASSOC)) {
    ...
    $fonction = $row['fonction_ID']." | ".$row['libelleFonct'];
    $deb = dateFR($row['dateDebut']);
    $fin = dateFR($row['dateFin']);

    $pdf->LigneTableau($i,$fonction,$deb,$fin);
    $i++;
}

/*------------------------------------------------------------
  9) Signature et sortie PDF
-------------------------------------------------------------*/
$pdf->Ln(12);
