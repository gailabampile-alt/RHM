<?php
    ...
    try {
        ...
        if(isset($_GET['id_act'])){
            $reqActiverGrade = $db->prepare ("UPDATE bdd_paie.t_type_doc SET statut = :statut,modifierPar =:modifierPar ,dateModif=:dateModif
            ...
            $reqActiverGrade->bindValue(':id_typedoc',$_GET['id_act']);
        ...
        if(isset($_GET['id_desac'])){
            ...
            $reqActiverGrade->bindValue(':modifierPar',$_SESSION['id_utilisateur']);
            $reqActiverGrade->bindValue(':dateModif',date('Y-m-d'));
            $reqActiverGrade->bindValue(':id_typedoc',$_GET['id_desac']);
            $reqActiverGrade->execute();
        ...
        if(isset($_POST['update_type_doc'])){
            $id_typedoc = validation_donnees($_POST['id_typedoc']);
            ...
            if(!empty($id_typedoc) || !empty($lib) )
            {
                try {
                   ...
                   $reqAddFonction = $db->prepare ("UPDATE bdd_paie.t_type_doc SET libelle_typedoc =:libelle_typedoc,modifierPar = :modifierPar,dateModif=:dateModif
                    WHERE id_typedoc =:id_typedoc");
                    $reqAddFonction->bindValue(':id_typedoc',$id_typedoc);
                    $reqAddFonction->bindValue(':libelle_typedoc',$lib);
