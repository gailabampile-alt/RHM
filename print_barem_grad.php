...
function BasicTable($header, $data)
    ...
    foreach($data as $row) {
        foreach($row as $index => $col) {
            $width = $this->GetStringWidth($col) + 21; // Ajouter une marge
    ...
    foreach($data as $row) {
        foreach($row as $index => $col) {
            ...
            $this->Cell($widths[$index], 7, $col, 1,0);
        }
        $this->Ln();
    }
    }
...
*/
    $header = array('N°','GRADE','LIBELLE BAR','MONTANT','DEVISE','STATUT');
    $compt = 1;
    // Data loading
    ...
    while($resbareme=$bareme->fetch()){
        ...
        $data[] = array($compt,$grade,$LibelleBar,$Mont,$code_devise,$statut);      
        ++$compt;       
    }
