    ...
    if(isset($_POST['CreerPointage'])){ 
       ...
       // $ph=$_POST['ph'];
        ...
        if(isset($periode)&& isset($matriAg)&& isset($datep)&& isset($nbrejrs)){ 
            ...
            $reqVerificationCodePaie = $db->prepare('SELECT * FROM bdd_paie.t_pointage WHERE periode=:periode and 	matric=:matricule ');
            ...
            if($reqVerificationCodePaie ->rowCount() == 1){
                $_SESSION['message']  = "Le Matricule : $matriAg a ete déjà pointé  pour la periode du : $periode !";
                $_SESSION['typeMsg']  = "danger";
                header('location:accueil.php?page=Pointage');
                exit();
                ...
                $reqAdd_CodePaie = $db->prepare('INSERT INTO bdd_paie.t_pointage( periode, datep, matric, nbrejrs,  creerpar, datecreat, modifierpar, datemodif) 
                VALUES (:periode,:datep,:matricule,:nbreh,:creerPar,:dateav,:modifierPar,:datemodif)');
          ...
          //  $reqVerificationCodePaie -> bindValue(':matricule',$matriAg);
           // $reqVerificationCodePaie -> bindValue(':datep',$datep);
            ...
            if($reqVerificationCodePaie ->rowCount() >= 1){
                ...
                $agents = $db->query('SELECT * FROM bdd_paie.t_agent inner join  bdd_paie.detail_agent_fonction on t_agent.matricule=detail_agent_fonction.agent_ID INNER JOIN  bdd_paie.t_fonction on  t_fonction.codeFonct=detail_agent_fonction.fonction_ID WHERE fonction_ID="0050" AND t_agent.activiter_ID="01"');
                $agents->execute();  
                $nbrejr="30";
