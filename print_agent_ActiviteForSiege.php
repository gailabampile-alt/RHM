...
$activiter = validation_donnees($_GET['activ']);
$siege = validation_donnees($_GET['siege']);
$req = $db->prepare("SELECT * FROM bdd_paie.t_activite where code_activ	= :code_activ");
$req -> bindValue(':code_activ',$activiter);
$req -> execute();
$resultat = $req->fetch(PDO::FETCH_ASSOC);

$lib_activite = $resultat['libelle_activ'];
...
class PDF extends FPDF
{
    ...
    function header_data($siege, $lib_siege,$lib_activ)
    {
        // En-tête spécifique au siège
        $this->SetFont('Arial','B',12);
        $txt = "L I S T E    D E S    A G E N T S    ".strtoupper($lib_activ)."    A    L A    D A T E    D U  ".date('d/m/Y');
...
$reqInfoAgent = $db->prepare("SELECT * FROM bdd_paie.v_agent_seiegact
    WHERE activiter_ID =:activiter_ID AND code_sieg =:code_sieg  ORDER BY matricule;");
$reqInfoAgent->bindValue(':activiter_ID',$activiter);
$reqInfoAgent->bindValue(':code_sieg',$siege);
...
while ($row = $reqInfoAgent->fetch()) {
    if ($current_siege !== $row['code_sieg']) {
        ...
        $pdf->AddPage();
        $pdf->header_data($row['code_sieg'], $row['libelle_sieg'],$lib_activite);
