...
<h3><i class="fa fa-angle-right"></i> Liste Des Agents / Syndicat</h3>
        <div class="row mb">
          <!-- page start-->
          ...
          <?php } ?>
            ...
            <a target="_blank" href="print_agent_syndicat_act.php" class="btn btn-round btn-primary col-sm-2" style="margin-bottom:25px;margin-left:15px;width:170px;"><i class="fa fa-print" style="margin-right: 7px;"></i>Imprimer</a>
                ...
                <tbody>
                  ...
                  <tr class="gradeC_">
                    <?php
                        ...
                        agsy.agent_ID,agsy.syndicat_ID,agsy.dateDebut,agsy.dateFin,agsy.statut_ID,agsy.creerPar,agsy.modifierPar,
                        CONCAT(ag.nom_ag,' ',ag.postnom_ag,' ',ag.prenom_ag) AS noms,ag.activiter_ID,
                        sy.libelle_syndi
                        FROM bdd_paie.detail_agent_syndicat AS agsy
                        INNER JOIN bdd_paie.t_agent AS ag ON agsy.agent_ID = ag.matricule
                        INNER JOIN bdd_paie.t_syndicat AS sy ON agsy.syndicat_ID = sy.code_syndi
                        WHERE ag.activiter_ID = '01' AND agsy.statut_ID = 'act' ");
                        $reqAgent_Siege ->execute();
                        while($resAgent_Siege=$reqAgent_Siege->fetch()){
                            $matric = $resAgent_Siege['agent_ID'];
                            $noms = $resAgent_Siege['noms'];
                            $code_gr = $resAgent_Siege['syndicat_ID'];
                            $lib_gr = $resAgent_Siege['libelle_syndi'];
                    ...
                    <td class="hidden-phone">
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li><a href="#accueil.php?page=<?php echo $code_dir;?>" style="color:darkblue"> <i class="fa fa-print" style="margin-right: 2px;"></i> Imprimer</a></li>
                                <!--li class="divider"></li>
