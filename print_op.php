...
*/
    ...
    // Data loading
    $reqGet = $db->prepare('SELECT * FROM bdd_paie.t_paie WHERE codeEiPaie = "999" ORDER BY codesiege, Matricule');
    $reqGet ->execute();
    while($row=$reqGet->fetch()){
        ...
        $data[] = array($row['EquiG'], $row['Matricule'], $row['Nom'].' '.$row['PostNom'], $row['numCompt'], $row['montant_payer'], '');
