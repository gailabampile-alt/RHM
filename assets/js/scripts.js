...
$(document).ready(function(){
	...
	//calculer la date fin bloque
	$('#DureBloque,#typebloque').change(function(){
		calculedatefin();
	});
...
function showHint(str) {
	var M = document.getElementById("MontantNetPaie");
  if (str == "" | M == '') {
    document.getElementById("MontanDevise").innerHTML = "";
    return;
  } else {
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("MontanDevise").innerHTML = this.responseText;
...
function showClient(str) {
  if (str == "") {
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         document.getElementById("Compte").innerHTML = this.responseText;
         
      }
    };
    xmlhttp.open("GET","CompteEntreprise.php?q="+str, true);
    xmlhttp.send();
  }

}

function showCompte(str) {
  if (str == "") {
      document.getElementById("CompteClient").innerHTML = "";
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         document.getElementById("CompteClient").innerHTML = this.responseText;
         
      }
    };
    xmlhttp.open("GET","TypeCompte.php?cpt="+str, true);
    xmlhttp.send();
  }

}

function showretrait(str) {
  if (str == "") {
      document.getElementById("retrait").innerHTML = "";
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         document.getElementById("retrait").innerHTML = this.responseText;
         
      }
    };
    xmlhttp.open("GET","retrait.php?ret="+str, true);
    xmlhttp.send();
  }

}

function showSiege(str) {
  if (str == "") {
      document.getElementById("Siege").innerHTML = "";
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         document.getElementById("Siege").innerHTML = this.responseText;
         
      }
    };
    xmlhttp.open("GET","SelectSiege.php?ret="+str, true);
    xmlhttp.send();
  }
...
function showProv(str) {
  if (str == "") {
      document.getElementById("Guichet").innerHTML = "";
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         document.getElementById("Guichet").innerHTML = this.responseText;
         
      }
    };
    xmlhttp.open("GET","SelectGuichetProv.php?ret="+str, true);
    xmlhttp.send();
  }
...
function calculesolde(){
	var Montpret= $('#Detailspret').val();
	var Tauxpret = $('#Detailstaux').val();
	var MontTaux = Number(Montpret) * Number(Tauxpret)/100;
	$('#DetailSolde').val(Number(Montpret) + Number(MontTaux));
}

function calculeretenir(){
	var solde= $('#DetailSolde').val();
	var Duree = $('#DetailsDuree').val();
	
	$('#Detailsretenir').val(Number(solde) / Number(Duree));
}
...
//calcule la date fin bloque

function calculedatefin(){
	var datedebut = Date.now();
	var durebloque = $('#DureBloque').val();
	var typebloque =$('#typebloque').val();
	var mois = datedebut.getMonth(); 
	
	$('#datefin').val(Number(datedebut)) ;
}
