...
class PDF extends FPDF
{
    ...
    function header_data($siege, $lib_siege)
    {
        ...
        $txt = "L I S T E    D E S    S T A G I A I R E S   A    L A    D A T E    D U  ".date('d/m/Y');
...
$reqInfoAgent = $db->prepare("SELECT * FROM bdd_paie.t_stagiare 
                INNER JOIN bdd_paie.t_siege ON t_siege.code_sieg = t_stagiare.siege_stg
                INNER JOIN bdd_paie.t_direction ON t_direction.code_dir = t_stagiare.dir_stg ORDER BY id_stg");
$reqInfoAgent->execute();

// En-tête du tableau
$header = array('CODE','NOM COMPLET', 'SEXE', 'DATE NAIS', 'DEBUT STG','FIN STG');
...
while ($row = $reqInfoAgent->fetch()) {
    ...
    $data[] = array($row['id_stg'], $row['nom_stg'].' '.$row['postnom_stg'].' '.$row['prenom_stg'], $row['sexe_stg'], $row['dateNaiss'], $row['dateDebut_stage'], $row['dateFin_stg']);
