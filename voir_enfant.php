              ...
              <table  cellpadding="0" cellspacing="0" border="0" class="display table table-bordered" id="hidden-table-info">
                <thead>
                  <tr>
                    ...
                    <th>Nom Parent</th>
                    <th>Matricule</th>
                    <th class="hidden-phone">Nom, Postnom & Prénom Enfant</th>
                    <th>Nbre Enfant</th>
                ...
                <tbody>
                  ...
                  <tr class="gradeC_">
                    ...
                    <td> <?php 
                        $nomComplet_Enf = "";
                        $reqEnfant = $db->prepare("SELECT * FROM bdd_paie.t_enfants_agent WHERE agent_ID = :agent_ID");
                        $reqEnfant->bindValue(':agent_ID',$matricule);
                        $reqEnfant->execute();
                        while ($resEnfant=$reqEnfant->fetch()) {
                          $nomComplet_Enf = $resEnfant['nom_enf'].' '.$resEnfant['postnom_enf'].' '.$resEnfant['prenom_enf'];
                          echo "<li>.$nomComplet_Enf.</li>";
                          echo "<br>";
                        }
                        $nbreEnfant = $reqEnfant->rowCount();?> 
                    </td>
                    
                    <td> <?php echo $nbreEnfant;?> </td>
                    <td> <?php 
                        $nomComplet_Enf = "";
                        $reqEnfant = $db->prepare("SELECT * FROM bdd_paie.t_enfants_agent WHERE agent_ID = :agent_ID");
                        $reqEnfant->bindValue(':agent_ID',$matricule);
                        ...
                        while ($resEnfant=$reqEnfant->fetch()) {
                          //$nomComplet_Enf = $resEnfant['nom_enf'].' '.$resEnfant['postnom_enf'].' '.$resEnfant['prenom_enf'];
                          ?>
                            <li> <a href="<?php echo $acte_naiss;?>"> Donwload</a> </li>
                            <br>
                          <?php
                        }
                        $nbreEnfant = $reqEnfant->rowCount();?> 
