...
class PDF extends FPDF
{
    ...
    {
        $widths = [15, 55, 60, 35, 25, 25, 30, 30];
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($widths[4], 7, iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $row[3]), 1);
            $this->Cell($widths[5], 7, iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $row[4]), 1);
            $this->Cell($widths[6], 7, iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $row[5]), 1);
            $this->Cell($widths[7], 7, iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $row[6]), 1);
...
$titre = "L I S T E  D E S  C O N G E S";
...
while ($row = $reqFonction->fetch()) {
    $data[] = [
        $row['nom_ag'] . ' ' . $row['postnom_ag'],
        $row['libelle_conge'],
        $row['excercice'],
        $row['nbre_jour'],
        ($row['statut'] === 'act' ? 'Active' : ($row['statut'] === 'desac' ? 'Désactive' : $row['statut'])),
...
$pdf->TablePro(
    ['N°', 'Nom Complet', 'Type de Congé', 'Exercice', 'Nbre Jour', 'Statut', 'Date Début', 'Date Fin'],
    $data
);
...
$pdf->SetTextColor(0, 0, 80);
$pdf->Cell(0, 8, 'Total : ' . count($data) . iconv('UTF-8', 'ISO-8859-1//TRANSLIT', ' Congé'), 0, 1, 'R');
