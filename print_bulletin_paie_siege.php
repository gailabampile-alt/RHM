...
*/
    ...
    // Data loading
    $result = $db->prepare("SELECT * FROM bdd_paie.t_paie WHERE codesiege ='$code_siege' AND periode ='$periode' ORDER BY Matricule");
    $result ->execute();
    $last_matric = NULL;
    $data = array();
    $test = 1;
    ...
    while($row = $result->fetch())
    {
        ...
        if ($last_matric !== $row['Matricule'] && $last_matric !== NULL) {
            ...
            $pdf->Cell(190,10,iconv('UTF-8', 'iso-8859-1',$txt),0,0,'L');

            $pdf->ln(0);
            $txt = "LAMA LUTSHIMA BIENVIEN";
            $pdf->SetFont('Arial','B',12);
            $pdf->Cell(190,10,iconv('UTF-8', 'iso-8859-1',$txt),0,0,'R');
