...
function Header()
{
    ...
    $this->Cell(30);
    $this->SetFont('Arial','B',12);
    $this->Cell(130,3,iconv('UTF-8', 'iso-8859-1',
...
$pdf->Cell(85);
$txt = " T  A  U  X ";
$pdf->Cell(35,10,iconv('UTF-8', 'iso-8859-1',$txt),1,0,'C');
/*
$pdf->ln(10);
...
*/
    $compt = 1;
    $header = array('N°', 'DATE', 'MONTANT','DEVISE','STATUT');
    // Data loading
    $result = $db->prepare('SELECT  * FROM bdd_paie.t_taux ORDER BY dateTaux');
    $result ->execute();
    $last_matric = NULL;
    while($row = $result->fetch())
    {
        $date = $row['dateTaux'];
        $devise = $row['monnaie_ID'];
        $statut = ($row['statut_ID'] == 'act') ?"Activer":"Desactiver";
        $montant = $row['montantTaux'];
        
        $data[] = array($compt++,$date,$montant,$devise,$statut);
