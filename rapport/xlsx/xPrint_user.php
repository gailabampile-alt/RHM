<?php
    ...
    */
    $sheet->mergeCells('A1:F1'); // Fusionne les cellules A1 à H1
...
$sheet->getRowDimension(1)->setRowHeight(60);
    ...
    $sheet->getStyle('A1')->getAlignment()->setWrapText(true);
    */
    $sheet->mergeCells('A2:F2'); // Fusionne les cellules A2 à H2
    ...
    $headers = ['N°', 'Matricule', 'Nom Complets', 'Username', 'Rôle', 'Statut'];
    $sheet->fromArray($headers, null, 'A3');
    $sheet->setAutoFilter('A3:F3'); // Ajout du filtre ici
    $sheet->getStyle('A3:F3')->getFont()->setBold(true);
    $sheet->getStyle('A3:F3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('A3:F3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('D9D9D9');


    $reqInfoAgent = $db->prepare('SELECT * FROM bdd_paie.v_liste_utilisateur ORDER BY id_user');
    ...
    while ($resInfoAgent=$reqInfoAgent->fetch()){
        ...
        $username = $resInfoAgent['username'];
        $statut = $resInfoAgent['libelle_st'];
        $role = $resInfoAgent['libelle_role'];
        
        

        $sheet->setCellValue("A".$row, $counter);
        $sheet->setCellValue("B".$row, $matric);
    ...
    $sheet->getStyle("A1:F1" . ($row - 1))->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
    $fileName = "rapport_".$_SESSION['id_utilisateur']."_".date('Ymd').".xlsx";
