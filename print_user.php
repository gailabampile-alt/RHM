...
class PDF extends FPDF
{
    ...
    {
        $widths = [15, 20, 80,80, 25,60]; // N°, Code, Libellé
...
$reqFonction = $db->prepare(
    'SELECT * FROM bdd_paie.v_liste_utilisateur ORDER BY id_user');
$reqFonction->execute();

while ($row = $reqFonction->fetch()) {
    $data[] = [
        $row['agent_ID'],
        $row['nom_ag'].' '.$row['postnom_ag'].' '.$row['prenom_ag'],
        $row['username'],
        $row['libelle_st'],
        $row['libelle_role']
    ];
...
// =============================
$pdf->TablePro(
    ['N°','Matricule','Nom Complet', 'Username', 'Statut', 'Rôle'],
    $data
);
